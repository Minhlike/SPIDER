...
const version = "spider-uncover/v1.2.1-secure2"
const valid = "VALID"
...
type report struct {
	...
	RequestJournal []requestJournalEntry `json:"request_journal"`
}
...
func classify(status int, body []byte) (string, string) {
	...
	if status == 401 {
		return invalid, "AUTH_REJECTED"
	}
	if status == 402 || status == 429 {
		return limited, "PLAN_OR_QUOTA"
	}
	...
	if status == 403 {
		return limited, "PERMISSION_DENIED"
	}
	...
	if status < 200 || status >= 300 {
		return network, "HTTP_ERROR"
	}
	if data == nil {
		return network, "UNEXPECTED_RESPONSE"
	}
...
func (g *guardedTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err != nil {
		g.journal[journalIndex].Outcome = "CONNECTION_FAILED"
		g.state, g.reason = network, "CONNECTION_FAILED"
		return nil, errors.New("connection failed")
	}
	g.status = response.StatusCode
	g.journal[journalIndex].HTTPStatus = response.StatusCode
	g.journal[journalIndex].Outcome = fmt.Sprintf("HTTP_%d", response.StatusCode)
...
func validSearchBody(host string, body []byte) bool {
	...
	case hosts["shodan"]:
		...
		for _, value := range rows {
			...
			if ip, exists := row["ip_str"]; exists {
				if _, ok := ip.(string); !ok {
					return false
				}
			}
			if port, exists := row["port"]; exists {
...
func validKeyFormat(engine string, keys map[string]string) bool {
	...
	default:
		return !strings.ContainsAny(keys["SHODAN_API_KEY"], "&?#%")
	}
}
...
func accountCheck(ctx context.Context, engine string, keys map[string]string, client *http.Client) (string, string, string) {
	...
	switch engine {
	case "shodan":
		address = "https://api.shodan.io/api-info?" + url.Values{"key": {keys["SHODAN_API_KEY"]}}.Encode()
	case "censys":
		address = "https://api.platform.censys.io/v3/accounts/users/credits"
	case "fofa":
		address = "https://fofa.info/api/v1/info/my?" + url.Values{"key": {keys["FOFA_KEY"]}}.Encode()
	}
	req, err := http.NewRequestWithContext(ctx, "GET", address, nil)
	if err != nil {
		return invalid, "MALFORMED_CREDENTIAL", ""
	}
	req.Header.Set("Accept", "application/json")
	if engine == "censys" {
		req.Header.Set("Authorization", "Bearer "+keys["CENSYS_API_TOKEN"])
		if keys["CENSYS_ORGANIZATION_ID"] != "" {
			req.Header.Set("X-Organization-ID", keys["CENSYS_ORGANIZATION_ID"])
		}
	}
	...
	case "shodan":
		...
		if credits == 0 && limits["query_credits"] != float64(-1) {
			return limited, "NO_QUERY_CREDITS", ""
		}
	case "censys":
		result, _ := data["result"].(map[string]any)
		if _, ok := result["balance"].(float64); !ok {
			return network, "UNEXPECTED_RESPONSE", ""
		}
	...
	case "fofa":
		...
		if saved, ok := keys["FOFA_EMAIL"]; ok && saved != "" {
			if returned, ok := data["email"].(string); ok && !strings.EqualFold(returned, saved) {
				warning = "ACCOUNT_EMAIL_MISMATCH"
			}
		}
...
func search(ctx context.Context, engine, query string, limit int, keys map[string]string, g *guardedTransport) report {
	...
	if engine == "fofa" && k.FofaEmail == "" {
		k.FofaEmail = "uncover-compat@invalid.local"
	}
	...
	for {
		select {
		case <-ctx.Done():
			result.State, result.Reason = network, "TIMEOUT"
			return result
		case item, ok := <-channel:
			...
			if item.Error != nil {
				result.State, result.Reason, result.HTTPStatus = providerState(engine, "search", g.state, g.status), g.reason, g.status
				if result.State == valid && result.Reason != "PAGE_LIMIT" {
					result.State, result.Reason = network, "UNEXPECTED_RESPONSE"
				}
				return result
			}
			...
			for _, secret := range keys {
				if secret != "" && (bytes.Contains(encoded, []byte(secret)) || bytes.Contains(encoded, []byte(url.QueryEscape(secret)))) {
					leak = true
				}
			}
			if !leak {
...
func main() {
	...
	result := report{Engine: *engine, State: missing, Reason: "REQUIRED_FIELDS_MISSING", Scope: *mode,
		Results: []map[string]any{}, RequestJournal: []requestJournalEntry{}}
	required, ok := fields[*engine]
	if !ok || (*mode != "check" && *mode != "search") || *limit < 1 || *limit > 100 {
		os.Exit(2)
	}
	...
	if *mode == "check" {
		...
		result = search(ctx, *engine, *query, *limit, keys, g)
	}
	result.RequestJournal = append([]requestJournalEntry(nil), g.journal...)
	json.NewEncoder(os.Stdout).Encode(result)
}
