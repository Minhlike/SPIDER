...
def client(monkeypatch):
    ...
    return TestClient(create_app(), base_url="http://127.0.0.1:8876")
...
@pytest.mark.parametrize("path,method", [("/", "GET"), ("/api/cases", "GET"),
    ...
    ("/api/runs/fixture/browser-challenges/continue", "POST"),
    ("/api/providers/native_dns/test", "POST"), ("/api/settings", "POST"),
    ("/api/system/shutdown", "POST")])
@pytest.mark.parametrize("headers", [{"host": "attacker.test:8876"},
    {"host": "127.0.0.1.attacker.test"}, {"host": "attacker.test@127.0.0.1:8876"},
...
def test_local_browser_and_cli_allowed(client):
    ...
    assert client.post("/api/classify", json={"target": "fixture-user"},
        headers={"origin": "http://127.0.0.1:8876", "sec-fetch-site": "same-origin"}).status_code == 200
...
@pytest.mark.parametrize("headers", [{}, {"origin": "https://attacker.test"},
    {"origin": "http://127.0.0.1:8876", "host": "attacker.test"}])
def test_hostile_websocket_rejected(client, headers):
    with pytest.raises(WebSocketDisconnect) as error:
        with client.websocket_connect("ws://127.0.0.1:8876/ws", headers=headers):
            pytest.fail("Hostile websocket accepted")
    assert error.value.code == 1008


def test_same_origin_websocket_allowed(client):
    with client.websocket_connect("ws://127.0.0.1:8876/ws", headers={"origin": "http://127.0.0.1:8876"}) as websocket:
        websocket.send_text("ping")


def test_shutdown_refuses_unmanaged_process(client):
    response = client.post("/api/system/shutdown", headers={
        "origin": "http://127.0.0.1:8876", "sec-fetch-site": "same-origin"})
