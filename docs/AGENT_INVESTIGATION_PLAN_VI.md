...
- M1 PARTIAL: scoped digest/evidence/graph, compare/coverage, benchmark có dữ liệu. Snapshot/delta và continuation của history/graph đã có; traversal chỉ đọc branch reachable thay vì toàn case. Hard bound cho reachable branch và continuation run comparison còn thiếu.
