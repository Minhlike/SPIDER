...
Quyết định vận hành local đã được người dùng duyệt: tiếp tục dùng hồ sơ Cốc Cốc
hiện có qua persistent context, không xuất cookie. Khi nhận diện CAPTCHA, SPIDER
giữ nguyên tab, chỉ công khai tên nguồn và trạng thái `HUMAN_REQUIRED` trên UI;
người dùng xử lý trong tab rồi bấm tiếp tục để tải lại đúng bước. Mặc định mỗi
origin chỉ có một request đang chạy; `Retry-After` và cooldown lũy tiến được giữ
qua lần khởi động trong dữ liệu local. Không dùng proxy hoặc dịch vụ giải CAPTCHA.
...
| P0-C rule registry/planning/proof graph | **EXPERIMENT RESULT — fixture PASS: versioned fail-closed edge/question registries, capability-aware PARTIAL coverage, claim lifecycle, scoped justification DAG, action scope/evidence context và deterministic plan preview; cùng snapshot tái hiện cùng plan/fingerprint, rebuild tái hiện materialized proof DAG. ACCOUNT → ACCOUNT chỉ thành POSSIBLY_SAME_IDENTITY khi observation có relationship_basis được rule 2.0.0 cho phép; thiếu predicate hạ xuống ASSOCIATED_WITH. DNS TTL tạo REVALIDATION_DUE nhưng currentness vẫn UNKNOWN. Temporal content chỉ tạo CHANGED/DISAPPEARED khi có contract complete, cùng provider/version/config/source scope và positive baseline tương đương; khác provider hoặc thiếu contract giữ OBSERVED/UNVERIFIED_ABSENCE_SIGNAL. Nguồn không có freshness contract vẫn NOT_INFERRED** |
...
| Browser reliability tradeoffs 1A/2B/3A/4A | **EXPERIMENT RESULT — fixture PASS ngày 14/09/2026: hồ sơ Cốc Cốc hiện có tiếp tục được dùng; CAPTCHA giữ tab và chờ nút tiếp tục của đúng run; API không trả target/URL/page/profile data; mỗi origin mặc định một request đang chạy; Retry-After tối đa 24 giờ và cooldown lũy tiến được lưu local; hostile Host/cross-origin bị từ chối; không tích hợp proxy hoặc CAPTCHA solver. Full suite 574 PASS. CAPTCHA live và manual-parity vẫn NOT YET VERIFIED** |
...
| P2 source-adoption benchmark gate | **EXPERIMENT RESULT — fixture scorer PASS; runner JSONL chỉ xuất aggregate, không echo row/fixture ID và reject field ngoài schema. Email benchmark tách OVERLAP accuracy khỏi HOLEHE_ONLY incremental coverage; username manual parity giữ UNKNOWN/BLOCKED ngoài negative. Chưa có consented holdout nên Holehe/Socialscan/nguồn VN mới đều NOT YET VERIFIED và chưa provider nào được ADOPT** |
