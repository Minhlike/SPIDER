...
| Username vertical slice v2 | **EXPERIMENT RESULT — fixture PASS cho direct route → search lead → mở lại đúng profile với tối đa 3 tab → URL → ACCOUNT lineage; liên kết tìm kiếm chưa mở lại chỉ là URL, kết quả bị chặn được giữ PARTIAL, identity ownership vẫn chưa xác minh. Outbound/reciprocal proof và live consented holdout chưa kiểm chứng** |
