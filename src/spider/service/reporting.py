...
VOCABULARY = {
    ...
    "SATISFIED": ("Các câu hỏi trong phạm vi này đã có bằng chứng trả lời", "Questions in this scope have evidence answers"),
    ...
    "OWNERSHIP_HYPOTHESIS": ("Giả thuyết tài khoản cần đối chiếu", "Account ownership hypothesis to corroborate"),
    "ACCOUNT_MAY_RELATE_TO_SEED": ("Tài khoản có thể liên quan tới mục tiêu", "Account may relate to the target"),
    "PROFILE_CANDIDATE": ("Mới ghi nhận trang hồ sơ", "Profile page observed"),
    "SELF_ASSERTED_LINK_ONLY": ("Có liên kết một chiều do trang tự công bố", "One-way link published by the page"),
    "RECIPROCAL_LINK_AVAILABLE": ("Có liên kết qua lại; danh tính vẫn cần đối chiếu", "Reciprocal link available; identity still needs corroboration"),
    "COLLECT_SELF_PUBLISHED_LINKS": ("Tìm liên kết công khai do hồ sơ tự giới thiệu", "Collect links published by the profile"),
    "CHECK_RECIPROCAL_PUBLIC_LINK": ("Kiểm tra trang đích có liên kết ngược lại hay không", "Check whether the destination links back"),
    "COMPARE_INDEPENDENT_PROFILE_EVIDENCE": ("Đối chiếu thêm bằng chứng hồ sơ từ nguồn độc lập", "Compare additional profile evidence from an independent source"),
    "REVIEW_SOURCE_COVERAGE": ("Xem nguồn nào chưa kiểm tra được", "Review sources that could not be checked"),
    "NO_ACCOUNT_CANDIDATE": ("Chưa có ứng viên tài khoản", "No account candidate is available"),
    "PROFILE_WITHOUT_LINK_PROOF": ("Hồ sơ chưa có bằng chứng liên kết", "The profile has no link evidence yet"),
    "ONE_WAY_SELF_ASSERTED_LINK": ("Mới có liên kết tự công bố một chiều", "Only a one-way self-published link is available"),
    "MAIL_INFRASTRUCTURE_NOT_PERSON_IDENTITY": ("Dữ liệu MX, SPF, DMARC và DNS chỉ mô tả hạ tầng thư; không xác định người đứng sau email", "MX, SPF, DMARC and DNS describe mail infrastructure; they do not identify the person behind an email"),
    "NETWORK_REGISTRANT": ("Đơn vị đăng ký/quản lý dải mạng", "Network range registrant or administrator"),
    "NETWORK_REGISTRY_METADATA": ("Metadata đăng ký dải mạng", "Network registry metadata"),
    "ROUTING_ORIGIN": ("Mạng công bố tuyến BGP", "BGP routing origin"),
    "ROUTING_ORIGIN_METADATA": ("Metadata mạng công bố tuyến BGP", "BGP routing-origin metadata"),
    "IP_INTELLIGENCE_ESTIMATE": ("Nhận định từ nguồn IP intelligence", "IP intelligence estimate"),
    "IP_GEOLOCATION_ESTIMATE": ("Vị trí ước lượng cấp mạng", "Network-level location estimate"),
    "NETWORK_OPERATOR_OR_EDGE_ONLY": ("Chỉ xác định được mạng vận hành hoặc vị trí edge", "Only the network operator or edge location is known"),
    "DATACENTER_NETWORK_CLASSIFICATION_ONLY": ("Nguồn phân loại IP thuộc mạng datacenter; chưa biết cơ sở vật lý cụ thể", "The source classifies the IP as datacenter network space; the physical facility is unknown"),
    "SOURCED_FACILITY_CLAIM": ("Có khai báo cơ sở vật lý từ nguồn chuyên biệt", "A specialized source reports a physical facility"),
